...
class Register extends AbstractAuth
{
    ...
    public function addTwoFactorRegister($user)
    {
        ...
        $registerMandatoryPhone = new RegisterMandatoryValue();

        if ($registerMandatoryPhone->isMandatory()) {
     ...
     *
     * @param object $errors Wordpress errors object
     */
    public function phoneValidationErrors($errors)
    {
        if (
            empty($_POST['2faPhone']) ||
            !empty($_POST['2faPhone']) && trim($_POST['2faPhone']) == ''
        ) {
            $registerMandatoryPhone = new RegisterMandatoryValue();
            if ($registerMandatoryPhone->isMandatory()) {
                if ($_POST['2faPhoneValidation'] != '') {
                    $errors->add('2faPhone', __('<strong>ERROR</strong>: You must include a valid phone number.'));
                } else {
                    $errors->add('2faPhone', __('<strong>ERROR</strong>: The phone number is required.'));
                }
            }
        } else {
            $registerMandatoryPhone = new RegisterMandatoryValue();
    ...
    public function showTwoFactorStep($userLogin, $userEmail, $errors = null)
    {
        ...
        $errors = self::phoneValidationErrors($errors);

        //Cathcup Wordpress validation first
        if (count($errors->errors) == 0) {

            if (!empty($_POST['2faPhone'])) {
